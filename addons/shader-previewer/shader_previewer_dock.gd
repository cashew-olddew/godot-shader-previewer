...
func _ready():
	...
	_last_floating_panel_size = Vector2(200.0, 200.0)
...
func _gui_input(event: InputEvent) -> void:
	...
	if _in_resize:
		...
		var code_edit: CodeEdit = get_parent() as CodeEdit
		if code_edit:
			moved_pos = moved_pos.clamp(Vector2.ZERO, _get_current_max_pos(code_edit))
		
		position = moved_pos

func _get_current_max_size(shader_code_editor: CodeEdit) -> Vector2:
	return shader_code_editor.size - Vector2(
			shader_code_editor.get_v_scroll_bar().size.x,
			shader_code_editor.get_h_scroll_bar().size.y
	)

func _get_current_max_pos(shader_code_editor: CodeEdit) -> Vector2:
	return shader_code_editor.size - (
		size + Vector2(
			shader_code_editor.get_v_scroll_bar().size.x,
			shader_code_editor.get_h_scroll_bar().size.y
	))

func resize_to_editor_shape(shader_code_editor: CodeEdit) -> void:
	var max_size: Vector2 = _get_current_max_size(shader_code_editor)
